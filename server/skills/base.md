...
### Format spécial `.pptx` (présentations)

Pour générer une **vraie présentation PowerPoint téléchargeable**, écris en markdown structuré, **les slides séparées par `---` sur une ligne seule** :

```
%%write_file:expose_seconde_guerre.pptx
# La Seconde Guerre Mondiale
## Une présentation pour le cours d'histoire

---

# Contexte
- Période : 1939 - 1945
- Acteurs principaux : Alliés vs Axe
- Théâtres : Europe, Pacifique, Afrique
...
**Règles `.pptx`** :
- 1ère slide = couverture (titre + sous-titre)
- Chaque `#` = titre de slide
- Chaque `##` = sous-titre (optionnel)
- Chaque `-` ou `*` = bullet point
- 4 à 8 slides max pour une présentation digeste
- Bullets concis (1 ligne chacun, pas de gros paragraphes)
