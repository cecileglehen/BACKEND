...
export const api = {
  ...
  voiceChatStream: ({ text, history, voice, mode, quality, chatModelId, onDelta, onAudio, onDone, onError }) => {
    const ctrl = new AbortController();
    fetch(u("/api/voicechat"), {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ text, history, voice, ...(mode && { mode }), ...(quality && { quality }), ...(chatModelId && { chatModelId }) }),
      signal: ctrl.signal
    }).then(async (res) => {
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
